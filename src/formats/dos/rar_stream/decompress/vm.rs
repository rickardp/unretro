...
use crate::formats::dos::rar_stream::crc32::crc32;

use super::byte_search;
...
impl RarVM {
    ...
    fn identify_filter(code: &[u8]) -> StandardFilter {
        ...
        if xor_sum != code[0] {
            #[cfg(test)]
            eprintln!(
                "    identify_filter: XOR checksum FAILED (expected 0x{:02x}, got 0x{:02x})",
                code[0], xor_sum
            );
            return StandardFilter::None;
        }
        ...
        #[cfg(test)]
        eprintln!(
            "    identify_filter: len={}, crc=0x{:08x}, checksum={}",
            code_len, code_crc, checksum_ok
        );
    ...
    fn read_data(data: &[u8], bit_pos: &mut usize) -> u32 {
        ...
        if bits_available < 2 {
            #[cfg(test)]
            eprintln!(
                "      read_data: EOF at bit_pos={}, data.len={}",
                *bit_pos,
                data.len()
            );
            return 0;
        ...
        if *bit_pos < 100 {
            eprintln!(
                "      read_data at bit {}: bytes[{}..{}]=[{:02x},{:02x},{:02x}], bit_off={}, val=0x{:04x}",
                *bit_pos,
                byte_pos,
                byte_pos + 3,
    ...
    pub fn add_code(
        ...
        #[cfg(test)]
        eprintln!(
            "  add_code: first_byte=0x{:02x}, code.len={}",
            first_byte,
            code.len()
        );
        ...
        let block_length = if (first_byte & 0x20) != 0 {
            ...
            #[cfg(test)]
            eprintln!(
                "    block_length from old_lengths[{}]: {}",
                filt_pos, self.old_lengths[filt_pos]
            );
            self.old_lengths[filt_pos]
        } else {
            #[cfg(test)]
            eprintln!(
                "    block_length: 0 (filt_pos {} >= old_lengths.len {})",
                filt_pos,
                self.old_lengths.len()
            );
            0
        };
        ...
        let filter_type = if new_filter {
            ...
            #[cfg(test)]
            eprintln!(
                "    new_filter: vm_code_size={}, bit_pos={}, code.len={}",
                vm_code_size,
                bit_pos,
                code.len()
            );
            ...
            #[cfg(test)]
            eprintln!(
                "    vm_code first 4 bytes: {:02x} {:02x} {:02x} {:02x}",
        ...
        #[cfg(test)]
        eprintln!(
            "    filter: type={:?}, block_start={} (raw {}+total_written {}), len={}",
        ...
        for (idx, filter) in self.stack.iter().enumerate() {
            ...
            if block_end <= total_written && filter.block_start < earliest_start {
                ...
                if filter.block_start < 1600000 && filter.block_start > 1400000 {
                    eprintln!(
                        "  find_ready_filter: found candidate idx={}, start={}, end={}, total_written={}",
    ...
    pub fn execute_filter_at_index(
        ...
        #[cfg(test)]
        eprintln!(
            "EXECUTE filter {:?}: start={}, len={}, total_written={}",
        ...
        #[cfg(test)]
        {
            if block_start == 46592 {
                ...
                eprintln!(
                    "  WINDOW at filter start: window[46592..46608] = {:02x?}",
                    (46592..46608)
                        .map(|p| window[p & window_mask])
                        .collect::<Vec<_>>()
                );
                // Show input bytes
                eprintln!("  INPUT to filter: mem[0..16] = {:02x?}", &self.mem[0..16]);
                eprintln!(
                    "  INPUT to filter: mem[9456..9472] = {:02x?}",
                    &self.mem[9456..9472]
                );
            }
        }
        ...
        #[cfg(test)]
        eprintln!(
            "  result: filtered_data_offset={}, filtered_size={}",
    ...
    pub fn execute_filters(
        ...
        #[cfg(test)]
        eprintln!(
            "EXECUTE filter {:?}: start={}, len={}, buffer.len={}",
            filter.filter_type,
            block_start,
            block_length,
            buffer.len()
        ...
        #[cfg(test)]
        eprintln!(
            "  result: filtered_data={}, filtered_size={}",
    ...
    fn filter_e8e9(
        ...
        // Word-at-a-time search for E8/E9 bytes (SWAR, ~8x faster than byte-by-byte)
        let search_end = data_size - 4;
        let mut cur_pos: usize = 0;

        while cur_pos < search_end {
            let found = if include_e9 {
                byte_search::find_byte2(&self.mem[cur_pos..search_end], 0xe8, 0xe9)
            } else {
                byte_search::find_byte(&self.mem[cur_pos..search_end], 0xe8)
            };
            if let Some(offset) = found {
                ...
                Self::transform_e8e9_addr(
                    &mut self.mem[addr_pos..addr_pos + 4],
                    addr,
                    offset_val,
                    FILE_SIZE,
                );
