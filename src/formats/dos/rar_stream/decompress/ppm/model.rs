...
impl PpmModel {
    ...
    pub fn decode_char(
        ...
        if num_stats != 1 {
            ...
            if self.debug_count == 0 {
                ...
                eprintln!(
                    "[pos={}] Binary context at {}, sym='{}' ({}), suffix={}, order_fall={}, max_order={}",
                    self.debug_count,
                    self.min_context,
                    sym as char,
                    sym,
    ...
    fn decode_bin_symbol(
        ...
        if self.debug_count == 0 {
            eprintln!(
                "[BIN pos={} idx] prev_success={} ns1={} hi_bits_flag={} hb2_flag[{}]={} run_length={}",
                self.debug_count,
                self.prev_success,
                ns1,
                self.hi_bits_flag,
                state.symbol,
                self.hb2_flag[state.symbol as usize],
                self.run_length
            );
            eprintln!(
        ...
        if count < bs as u32 {
            ...
            if self.debug_count == 0 {
                let (code, low, range) = coder.debug_state();
                eprintln!(
                    "[BIN pos={}] FOUND: lo=0 hi={} scale={} | before decode: low={} range={} code={}",
            ...
            if self.debug_count == 0 {
                let (code, low, range) = coder.debug_state();
                eprintln!(
                    "[BIN pos={}] ESCAPE: lo={} hi={} scale={} | before decode: low={} range={} code={}",
    ...
    fn decode_symbol2(
        ...
        if !is_root {
            ...
            if self.debug_count == 58 || self.debug_count == 0 {
                ...
                eprintln!(
                    "[DS2 pos={}] SEE2 inputs: i={} suffix_ns={} summ_freq={} num_masked={} hi_bits_flag={}",
                    self.debug_count, i, suffix_ns, summ_freq, self.num_masked, self.hi_bits_flag
                );
                eprintln!(
                    "[DS2 pos={}] SEE2 indices: ns2_idx={} diff_suffix={} freq_check={} masked_check={}",
        ...
        if self.debug_count == 15 {
            ...
            for j in 0..num_stats {
                ...
                if self.char_mask[sym as usize] != self.esc_count {
                    ...
                    if freq > 1 {
                        eprintln!(
                            "[DS2 pos=15] sym='{}' ({}) j={} prev_cum={} cum={} freq={} state_ptr={}",
    ...
    fn update_model(&mut self) {
        ...
        while pc != self.min_context {
            ...
            if ns1 != 1 {
                ...
                if pc == 15728580 {
                    eprintln!(
                        "[UPDATE_MODEL pos={}] context {} promoted: new_freq={} init_esc={} init_esc_extra={} → SummFreq={}",
