    ...
    pub fn alloc_units(&mut self, nu: usize) -> Option<usize> {
        ...
        if self.free_list[idx] != 0 {
            ...
            if (1024..2560).contains(&ptr) {
                eprintln!(
                    "[ALLOC] WARNING: alloc_units({}) from FREE LIST returned {} which overlaps root stats [1024, 2560)",
                    nu, ptr
                );
            }
        ...
        if self.lo_unit + bytes_needed <= self.hi_unit {
            ...
            if (1024..2560).contains(&ptr) {
                eprintln!(
                    "[ALLOC] WARNING: alloc_units({}) from LO_UNIT returned {} which overlaps root stats [1024, 2560)",
                    nu, ptr
                );
            }
        ...
        if (1024..2560).contains(&ptr) {
            eprintln!(
                "[ALLOC] WARNING: alloc_units({}) from RARE returned {} which overlaps root stats [1024, 2560)",
                nu, ptr
            );
        }
